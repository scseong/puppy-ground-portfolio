...
4. 모달 내부에서도 이전/다음 게시물 탐색 기능 제공
   ...
   - `getPrevAndNextPost` rpc를 통해 이전/다음 게시물 ID를 조회

   - `router.replace`를 사용하여 URL만 바꾸고 페이지 리로드 없이 모달 재렌더링

   - 이전/다음 버튼 마우스 오버 시 `router.prefetch`로 미리 데이터 불러오기

     ```tsx
       <button onClick={() => handleNavigate(next)} onMouseEnter={() => router.prefetch(`/mungstagram/${next}`)}>
...
- 페이지 전환 없이 **끊김 없는 상세 조회**, 뒤로 가기 버튼과 스크롤 유지로 **탐색 중단 없는 UX** 구현
- 개별 게시물 URL 공유 및 직접 진입 가능 → **SEO 문제 해결**
